# tests/integration_tests/test_gsheet_info_connection.py
import pytest
import json
import gspread
...
def test_google_sheet_info_connection():
    ...
    info_sheet_url = re.search(r"info_gdrive\s*=\s*\'(.+?)\'", lines).group(1)
    ...
    try:
        ...
        sheet = client.open_by_url(info_sheet_url).sheet1
        assert sheet.title is not None
    except Exception as e:
        pytest.fail(f"Failed to connect to Google Sheet (Info): {str(e)}")
